import {Box, Button, HStack, Icon, Input, Link, Text, useToast} from "@chakra-ui/react";
...
const Commentary = ({commentary}: Props) => {
  ...
  return (
    <Box paddingY="1vh">
        <HStack>
            <Text fontSize="md" color={"green.300"}>{commentary.creatorName}</Text>
            <Text fontSize="md" color={"darkgray"}>{moment(commentary.created).fromNow()}</Text>
        </HStack>
      <Text fontSize="md">{commentary.content}</Text>
        <HStack>
            ...
            <Link onClick={handleReplyClick}>
                <Text fontSize="sm" color="gray.300">Reply</Text>
            </Link>
        </HStack>
        ...
        {commentary.replies && commentary.replies.length > 0 && (
            <Link onClick={handleShowReplies}>
                <Text fontSize="sm" color="teal.300">Show/Hide replies</Text>
            </Link>
        )}

        {/* Render replies recursively */}
        {areRepliesVisible && commentary.replies && commentary.replies.length > 0 && (
            <Box paddingLeft="4">
